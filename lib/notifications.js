// Enhanced Notification Service with Database Logging and Email Templates
...
export async function sendOrderStatusNotification(order, newStatus) {
  ...
  const emailHTML = orderStatusTemplate(order, newStatus)

  const notification = await saveNotification({
    type: 'order_status',
    recipientEmail: order.customerEmail,
    recipientPhone: order.customerPhone,
    subject: 'Order Status Update - Mr. COCO Bakery',
    message,
    emailHTML,
    metadata: {
      orderId: order._id,
...
export async function sendWelcomeNotification(user) {
  ...
  const emailHTML = welcomeTemplate(user)
...
export async function sendBulkOrderNotification(bulkOrder) {
  ...
  const emailHTML = bulkOrderNotificationTemplate(bulkOrder)

  const notification = await saveNotification({
    type: 'bulk_order',
    recipientEmail: 'admin@mrcoco.com',
    recipientPhone: '+918447655399',
    subject: 'New Bulk Order Inquiry - Mr. COCO Bakery',
    message,
    emailHTML,
    metadata: {
...
export async function sendLowStockAlert(product) {
  ...
  const emailHTML = lowStockAlertTemplate(product)

  const notification = await saveNotification({
    type: 'low_stock',
    recipientEmail: 'admin@mrcoco.com',
    recipientPhone: '+918447655399',
    subject: 'Low Stock Alert - Mr. COCO Bakery',
    message,
    emailHTML,
    metadata: {
